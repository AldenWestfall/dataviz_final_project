...
```{r, message=FALSE, warning=FALSE, fig.alt="A revamped map of the U.S. showcasing the NBA Final Championship Wins by City. Each city is represented by a dot on the map that is scaled with the number of wins along with a more clear label."}
finals <- df %>%
  filter(Win == 1) %>%
...
```{r, message=FALSE, warning=FALSE,fig.alt="An interactive column chart that breaks down the final game stats of each year. The interactivity allows you to select each individual stat line."}
final_games <- df %>%
  filter(Win == 1) %>%
  group_by(Year) %>%
  slice_max(Game, n = 1) %>%
...
```{r, message=FALSE, warning=FALSE,fig,alt="A dot plot with a trend line depicting Points Scored Over Time Predicted vs Actual by Home/Away."}
...
```{r, message=FALSE, warning=FALSE,fig.alt="Summary of the primart model that gives statisical information about the first model"}
summary(model)
model2 <- lm(PTS ~ Year + Home + TRB + TOV + TPP, data = df)
```


```{r, message=FALSE, warning=FALSE,fig.alt="Summary of a secondary model that gives statisical information about model2"}
summary(model2)
```
...
```{r, message=FALSE, warning=FALSE, fig.alt="A map of the U.S. showcasing the NBA Final Championship Wins by City. Each city is represented by a dot on the map that is scaled with the number of wins along with a label."}
ggplot() +
  geom_sf(data = us_states, fill = "grey", colour = "white") +
  geom_sf(data = city_sf, aes(size = wins, colour = Team)) +
