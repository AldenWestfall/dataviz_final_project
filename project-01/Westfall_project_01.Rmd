...
```{r, message=FALSE, warning=FALSE, fig.alt="A bar chart showing the average IMDB rating for each genre seperated by years from 2012 to 2016."}
data %>%
...
```{r, fig.alt="A heatmap showcasing the number of movies each genre has had thorughout each year from 2012 to 2016"}
data %>%
  filter(Year>=2012 ) %>%
...
```{r, message=FALSE, warning=FALSE, fig.alt="A standard dot plot that compares the IMDB Rating to Revenue."}
data %>%
ggplot(aes(x= Rating, y= `Revenue (Millions)`)) +
  geom_point(alpha= 0.6, size = 2) +
