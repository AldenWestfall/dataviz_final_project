...
```{r, fig.alt="Multiple histogram charts for each month in 2022 that shows the number of times each max temperature was experienced."}
...
```{r, fig.alt="Density graph showing the number of spread of maximum temperature for 2022"}
weather_tpa2 %>%
...
```{r, fig.alt="Multiple density charts for each month visualizing the spread of max temperature experienced in 2022"}
weather_tpa2 %>%
...
```{r, fig.alt="Density ridge chart showing the spread of max temperature experienced in 2022 in each month"}
library(ggridges)
...
```{r, fig.alt="Column chart showing total precipitation in inches by month in Tampa for 2022"}
weather_tpa2 %>%
...
```{r, fig.alt="Bar chart showing the top 20 songs that use the word 'love' most in the Billboard Top 100 for 2015"}
df %>%
...
```{r, fig.alt="Interactive density ridge chart showing the spread of max temperature experienced in 2022 in each month. Rolling over a month provides summary statistics."}
weather_tpa %>%
  mutate(month = factor(month, levels = 1:12, labels = month.name)) %>%
